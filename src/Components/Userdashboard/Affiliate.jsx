...
function Affiliate() {
  ...
  useEffect(() => {
    if (orderdata.length > 0) {
      // This useEffect will run when orderdata changes
      axios
        ...
        <div className="card h-100 d-flex flex-column">
          ...
          <div className=" card-footer row mt-2">
            <div className="col-12 text-center">
              <a className="buttonlearnmore">
                <button
                  className=" learnmore w-75"
                  onClick={() => getAffiliate(productdata._id)}
                >
                  Get Affiliate Link
                </button>
              </a>
              {affiliateLinks[productdata._id] && (
                <>
                  <input
                    type="text"
                    className="w-75"
                    value={affiliateLinks[productdata._id]}
                    readOnly
                  />
                  {copied[productdata._id] ? (
                    <span className="ms-2 text-success">Copied!</span>
                  ) : (
                    <i
                      className="fa fa-copy ms-2"
                      onClick={() => handleCopyLink(productdata._id)}
                    ></i>
                  )}
                </>
              )}
  ...
  const getAffiliate = (courseId) => {
    ...
    axios
      .post(
        ROOT_URL + `/api/affiliate/generate/courselink`,
        { userId, courseId }
      )
      .then((response) => {
      ...
      {productdata.length > 0 ? (
        ...
        <div>
          <h3 className="text-center">
            Purchase any course to become an affiliate
          </h3>
        </div>
      )}
    </>
